...
class _MonitorScreenState extends State<MonitorScreen> {
  ...
  Future<void> start() async {
    ...
    _sub = _meter.noiseStream.listen((e) {
      final db = e.meanDecibel.isFinite ? e.meanDecibel : 0.0;
      final speaking = db > 55;
      if (speaking && !wasSpeaking) segments++;
      wasSpeaking = speaking;
      String s = "Calm";
      if (db >= loudDb) s = "LOUD";
      else if (segments >= fastCount) s = "FAST";
      setState(() {
        currentDb = db;
        status = s;
      });
      if (s != "Calm") Vibration.vibrate(duration: 200);
    });
    setState(() {
      listening = true;
      status = "Listening";
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(children: [
          ...
          const Spacer(),
          Row(children: [
            Expanded(child: ElevatedButton(onPressed: listening ? null : start, child: const Text("Start"))),
            const SizedBox(width: 12),
            Expanded(child: ElevatedButton(onPressed: listening ? stop : null, child: const Text("Stop"))),
          ]),
        ]),
      ),
    );
